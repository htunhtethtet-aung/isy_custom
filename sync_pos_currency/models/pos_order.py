...
class PosOrder(models.Model):
    ...
    def _payment_fields(self, order, ui_paymentline):
        result = super(PosOrder, self)._payment_fields(order, ui_paymentline)
        result.update({
            'selected_currency_rate': ui_paymentline.get('selected_currency_rate') or order.pricelist_id.currency_id.rate,
            'selected_currency_symbol': ui_paymentline.get('selected_currency_symbol') or order.pricelist_id.currency_id.symbol,
            'currency_amount_total': ui_paymentline.get('currency_amount_total') or ui_paymentline.get('amount', 0.0)
        })
        return result
